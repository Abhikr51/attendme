...
import { StyleSheet, View ,Dimensions ,ActivityIndicator , Button ,Image} from 'react-native';
...
const NoInternet = ({navigation,route}) => {
  ...
  return (
    <Screen style={{
      ...
      <AppText style={styles.text} > <Feather name='wifi-off' size={20} color={AppColors.black} /> No Internet </AppText>
      <Image source={require("../assets/images/signal.png")} style={styles.image} resizeMode="contain" />
      
      {(progress) ?
        <ActivityIndicator style={{position : 'absolute', bottom : 30}} size='large'  color={AppColors.primary} /> 
        : 
